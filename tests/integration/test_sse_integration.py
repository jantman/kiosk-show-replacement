...
class TestSSEIntegration:
    ...
    def test_react_authentication_flow(self, servers, enhanced_page: Page):
        ...
        try:
            ...
            enhanced_page.wait_for_selector("input[name='username'], input[placeholder*='username' i]", timeout=10000)
        ...
        assert login_form_visible or enhanced_page.locator("h1").is_visible(), \
        ...
        try:
            ...
            if "admin" in page_content.lower() or "dashboard" in page_content.lower():
                ...
                try:
                    ...
                    monitoring_link = enhanced_page.locator("text=System Monitoring").first
                    if monitoring_link.is_visible():
                        ...
                        if sse_elements.count() > 0:
                            ...
                            logger.info("No SSE elements found, but navigation successful")
                    else:
                        logger.info("System Monitoring link not visible, checking for other SSE indicators")
                except Exception as e:
                    logger.info(f"Navigation attempt failed: {e}, but basic page load successful")
            else:
    ...
    def test_basic_flask_backend_functionality(self, servers):
        ...
        try:
            ...
            assert response.status_code in [200, 302, 404]  # 302 for login redirect, 404 for API-only, 200 for success
        except Exception as e:
...
class TestSSEFailureScenarios:
    ...
    def test_server_logs_capture(self, servers):
        ...
        logger.info(f"Initial log counts - Flask: {initial_flask_logs}, Vite: {initial_vite_logs}")
        ...
        try:
            ...
            logger.info(f"Final log counts - Flask: {final_flask_logs}, Vite: {final_vite_logs}")
