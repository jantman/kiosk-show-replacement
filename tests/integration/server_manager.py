...
class EnhancedServerManager:
            ...
            for attempt in range(30):  # 30 second timeout
                try:
                    ...
                    if response.status_code in [200, 302, 404]:  # Any valid HTTP response
                        self.logger.info(f"✓ Flask server ready at {self.flask_url} (status: {response.status_code})")
                        return True
                except requests.exceptions.RequestException as e:
                    self.logger.debug(f"Flask not ready yet (attempt {attempt + 1}): {e}")
        ...
        try:
            ...
            for attempt in range(30):  # 30 second timeout
                try:
                    ...
                    self.logger.debug(f"Vite not ready yet (attempt {attempt + 1}): {e}")
    ...
    def _log_server_status(self):
        ...
        self.logger.info(f"Server status - Flask: {'✓' if health['flask'] else '✗'}, Vite: {'✓' if health['vite'] else '✗'}")
