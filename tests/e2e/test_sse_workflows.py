...
@pytest.mark.e2e
@pytest.mark.skip(
    reason="SSE E2E tests deferred - SSE functionality belongs in integration tests (React admin interface)"
)
class TestSSEWorkflows:
    ...
    def setup(self, page: Page, context: BrowserContext):
        ...
        with app.app_context():
            ...
            self.test_display = Display(
                name="Test Display", location="Test Location", owner_id=1
            )
        ...
        try:
            ...
            async with self.context.request.new_context() as api_context:
                ...
                if login_response.status == 200:
                    ...
                    assert (
                        response.status == 200
                    ), f"Heartbeat API failed with status {response.status}"
                else:
                    # If API login fails, just test that the dashboard loads
                    print(
                        f"API login failed with status {login_response.status}, testing dashboard only"
                    )
        ...
        try:
            ...
            async with self.context.request.new_context() as api_context:
                ...
                if login_response.status == 200:
                    ...
                    assert (
                        response.status == 200
                    ), f"Slideshow assignment API failed with status {response.status}"

                    # Verify assignment was saved by querying the API
                    get_response = await api_context.get(
                        f"{live_server.url}/api/v1/displays/{self.display_id}"
                    )
                    if get_response.status == 200:
                        display_data = await get_response.json()
                        assert (
                            display_data.get("current_slideshow_id")
                            == self.slideshow_id
                        ), "Slideshow assignment not saved"
                else:
                    # If API login fails, just test that the dashboard loads
                    print(
                        f"API login failed with status {login_response.status}, testing dashboard only"
    ...
    async def test_sse_connection_status_indicator(self, live_server):
        ...
        try:
            ...
            connection_indicator = self.page.locator(
                '[data-testid="sse-connection-status"]'
            )
            if await connection_indicator.count() > 0:
                try:
                    await expect(connection_indicator).to_contain_text(
                        "Connected", timeout=800
                    )
                except Exception:
            ...
            sse_errors = any(
                "error" in msg.lower() and "sse" in msg.lower()
            ...
            assert not (
                sse_errors and not sse_related
            ), f"SSE errors without connection attempt. Console: {console_messages[:5]}"
...
@pytest.mark.e2e
@pytest.mark.skip(
    reason="SSE E2E fallback tests deferred - requires SSE infrastructure debugging"
)
    ...
    async def test_sse_error_handling(self, page: Page, live_server):
        ...
        try:
            ...
            sse_errors = [
                msg
                for msg in console_messages
                if "SSE" in msg and "error" in msg.lower()
            ]
