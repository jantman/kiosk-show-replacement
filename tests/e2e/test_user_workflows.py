...
class TestUserAuthenticationE2E:
    ...
    def test_login_workflow_and_dashboard_verification(
        ...
        assert (
            login_form_visible
        ), "Login form should be visible for non-authenticated user"
        ...
        assert (
            not still_on_login
        ), "Login should have succeeded - should not be on login page anymore"
        ...
        if not dashboard_visible:
            ...
            assert (
                not still_on_login
            ), "Successfully logged in but cannot find dashboard indicators"
...
class TestUnauthenticatedAccessE2E:
    ...
    def test_unauthenticated_user_redirected_to_login(
        ...
        login_present = (
            page.locator("h4:has-text('Login')").is_visible()
        ...
        # Just verify that login page is working - authentication checks
