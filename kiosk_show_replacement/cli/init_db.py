...
Press <strong>Escape</strong> to exit this display mode
    ...
    for slide_data in sample_slides:
        slide = SlideItem(slideshow_id=welcome_slideshow.id, **slide_data)
...
Available 24/7 for technical support
    ...
    for slide_data in simple_slides:
        slide = SlideItem(slideshow_id=simple_slideshow.id, **slide_data)
        db.session.add(slide)
...
def main(sample_data, database_url, force):
    ...
    with app.app_context():
        ...
        if not force:
            try:
                ...
                if existing_tables:
                    ...
                    click.echo(
                        "   Use --force to reinitialize (this will not drop existing data)"
                    )
                    if not sample_data:
                        click.echo(
                            "   Use --sample-data to add sample data to existing database"
                        )
                        return
            except Exception:
        ...
        try:
            ...
            if sample_data:
                click.echo(
                    "  3. Check out the sample slideshows to see the system in action"
                )
            else:
                click.echo(
                    "  3. Create your first slideshow in the management interface"
                )
