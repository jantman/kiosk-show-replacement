...
class DatabaseUtils:
        ...
        try:
            ...
            for table in tables:
                try:
                    ...
                    table_status[table] = {"exists": True, "record_count": count_result}
                except Exception as e:
            ...
            return {
                ...
                "database_url": current_app.config.get(
                    "SQLALCHEMY_DATABASE_URI", "unknown"
                ),
                "tables": table_status,
    ...
    def create_admin_user(
        username: str = "admin", password: str = "admin", email: Optional[str] = None
    ) -> Optional[User]:
        """
        ...
        try:
            ...
            # Create admin user
            admin_user = User(
                username=username, email=email, is_active=True, is_admin=True
            )
            ...
            current_app.logger.error(
                f"Error creating admin user - integrity constraint: {e}"
            )
            return None
        except Exception as e:
            db.session.rollback()
        ...
        try:
            ...
            items2 = [
                SlideshowItem(
                    ...
                    content_type="text",
                    content_text="We are committed to excellence in digital "
                    "signage solutions.",
                    order_index=0,
                    created_by=user,
                ),
...
class BackupUtils:
    ...
    def create_sqlite_backup(backup_path: str) -> bool:
        ...
        try:
            ...
            if not db_url.startswith("sqlite:"):
                current_app.logger.error(
                    "Backup currently only supports SQLite databases"
                )
                return False
    ...
    def restore_sqlite_backup(backup_path: str) -> bool:
        ...
        try:
            ...
            if not db_url.startswith("sqlite:"):
                current_app.logger.error(
                    "Restore currently only supports SQLite databases"
                )
                return False
