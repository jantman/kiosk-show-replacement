    ...
    def add_event(self, event: SSEEvent) -> None:
        ...
        if self.is_active:
            try:
                ...
                logger.warning(f"Failed to queue event for connection {self.connection_id}: {e}")
...
class SSEManager:
    ...
    def create_connection(self, user_id: Optional[int] = None,
                         connection_type: str = "admin") -> SSEConnection:
...
def create_display_event(event_type: str, display_id: int, 
                        data: Dict[str, Any]) -> SSEEvent:
    ...
    return SSEEvent(
        event_type=f"display.{event_type}",
        data=event_data
    )


def create_slideshow_event(event_type: str, slideshow_id: int,
                          data: Dict[str, Any]) -> SSEEvent:
    ...
    return SSEEvent(
        event_type=f"slideshow.{event_type}",
        data=event_data
    )
